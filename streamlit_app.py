"""Streamlit entry point for the Skalu analyzer demo."""
...
if analysis:
    ...
    if is_pdf:
        ...
        total_lines = sum(len(page.get("horizontal_lines", [])) for page in analysis["result_data"].get("pages", []))
        total_rectangles = sum(len(page.get("rectangles", [])) for page in analysis["result_data"].get("pages", []))
