...
type QueryState = DetectionParams & {
  ...
  const handleProcess = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      await analyzeFileStream(
        {
          file: usingFileUrl ? null : selectedFile,
          file_url: usingFileUrl ? fileUrl : null,
        },
        {
        ...
        <form onSubmit={(event) => void handleProcess(event)}>
          ...
          <label style={{ display: "inline-flex", gap: "0.4rem", alignItems: "center", color: "#475569", cursor: "pointer" }}>
            <input
              ...
              onChange={(event) => updateQueryState({ ...queryState, include_empty_pages: event.target.checked })}
            />
          ...
          <label style={{ display: "inline-flex", gap: "0.4rem", alignItems: "center", color: "#475569", cursor: "pointer" }}>
            <input
              ...
              onChange={(event) => updateQueryState({ ...queryState, include_visualizations: event.target.checked })}
            />
