...
type AnalyzeInput = { file: File | null; file_url: string | null };
...
export const fetchVersion = async (): Promise<VersionResponse> => {
    const response = await fetch(`${API_BASE}/version`, {
        headers: { 'X-Frontend-Version': `${__APP_VERSION__}+${__APP_GIT_SHA__}` },
    });
    if (!response.ok) {
...
export const analyzeFile = async (
    ...
    const response = await fetch(`${API_BASE}/analyze`, { body: form, method: 'POST' });
    if (!response.ok) {
